...
contract CoffeeCoin {
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
        ...
        require(address(0) != to, "Use burn for this");
    ...
    function mint(address receiver, uint amount) public {
        ...
        require(msg.sender == _minter, "ERC20: only the creator of the smart contract can add new tokens to the balance");
         ...
         _totalSupply = safeAdd(_totalSupply, amount);  // Выпускаем
        balances[receiver] += amount; // Начисляем
        emit Transfer(address(0), receiver, amount); // Уведомляем
    }


    function burn(uint256 amount) public {
        require(balances[msg.sender] >= amount, "ERC20: balance[msg.sender] >= amount");
        require(_totalSupply >= amount, "ERC20: amount to burn should be less then _totalSupply coins");
        require(_totalSupply > 0, "ERC20: There aren't any avail tokens");
